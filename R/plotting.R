...
# returns vector from lo to hi with multiplication steps of incr. Used for
# making ticks to a log-scaled axis
seq.log <- function(from, to, base) {
  base^(log(from, base):log(to, base))
}

# returns vector of minor tick spacings appropriate for log 10 scaled axis with
# major ticks given by 'major'.
seq.log.minor <- function(major) {
  temp <- NULL
  if (length(major) > 1)
    for (i in 1:(length(major) - 1)) temp <- c(temp, seq(major[i], major[i +
      1] - major[i], major[i]))
...
axis.log10 <- function(side = 1, horiz = FALSE, labels = TRUE, baseAxis = TRUE,
  wholenumbers = TRUE, labelEnds = TRUE, las = 1, at = NULL) {
  ...
  # make labels
  if (labels)
    axis(side, at = at, lab, col = if (horiz)
      fg else NA, col.ticks = fg, las = las) else axis(side, at = at, FALSE, col = if (horiz)
