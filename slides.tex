\documentclass[20pt]{beamer}
\usepackage{array} % Used for > in Resources table

\input{modules/tex/style}
\input{modules/tex/beamer}
\input{modules/tex/positioning}
\usepackage{modules/tex/fontawesome}

\def\twitter{{\FA \faTwitter}}
\def\heart{{\FA \faHeart}}
\def\tickmark{{\FA \faCheck}}
\def\rarrow{{\FA \relsize{-1}{\faArrowRight}}}

\def\imagetop#1{\vtop{\null\hbox{#1}}}

\newcommand{\hrefp}[2]{\href{#1://#2}{#2}}
\newcommand{\twitterhandle}[1]{\href{https://twitter.com/#1}{\twitter #1}}
\newcommand{\twitterhandlesm}[1]{\relsize{-1}{\color{b-grey}\href{https://twitter.com/#1}{@#1}}}

\begin{document}

\begin{frame}
  \begin{tikzpicture}[remember picture, overlay]
    \node (A) at ($(current page.center) + (0, .17\paperheight)$) {
      \resizebox{.9\paperwidth}{!}{%
        \color{b-blue}\bf The challenges of combining 176 x \#otherpeoplesdata to create}
    };
    \node[anchor=north] at (A.south) {
      \resizebox{.9\paperwidth}{!}{%
        \color{b-green}the Biomass And Allometry Database (BAAD)}
    };
    \color{white}
    \node[anchor=south west] (B) at
    ($(current page.south west) + (0.05\paperwidth, 0.05\paperwidth)$) {
      \resizebox{.9\paperwidth}{!}{%
      \color{b-grey}\twitterhandle{adaptive\_plant},
       \href{mailto: daniel.falster@mq.edu.au}{{ \FA \faEnvelope} daniel.falster@mq.edu.au}, 
      \href{http://danielfalster.com}{{\FA \faHome} danielfalster.com}
      }
    };
    \node[anchor=south west] at (B.north west) {
      \resizebox{.9\paperwidth}{!}{%
        \color{white} 
        Daniel Falster, Rich FitzJohn, Remko Duursma, Diego Barneche}
    };
 \end{tikzpicture}
\end{frame}

%-----------------------------------
\begin{frame}\frametitle{\color{b-purple} Harvested plants}
\begin{tikzpicture}[remember picture, overlay]
  \node[anchor = south, align=center] at ($(current page.south) +(0,0.25cm)$) {
    \includegraphics<1>[width=0.95\textwidth]{pics/baad/Slide10.jpg}
  };
  \end{tikzpicture}
\end{frame}

%-----------------------------------
\begin{frame}\frametitle{\color{b-blue} A global compilation}

\begin{tikzpicture}[remember picture, overlay]
  \node[anchor = south, align=center] at ($(current page.south) +(0,1cm)$) {
    \color{white} 176 studies,  21084 plants, 678 species
  };

  \node[anchor = south, align=center, fill=white, rounded corners,
  draw=b-orange, line width=2pt] at ($(current page.south) +(0,2.5cm)$) {
    \includegraphics<1>[width=0.98\textwidth]{figures/map}
  };
  \end{tikzpicture}
\end{frame}

% %-----------------------------------

% \begin{frame}
%   \herohigh{\color{b-purple} Co-authored paper}
%   \bottomhanging{
%     \includegraphics<1>[width=.85\paperwidth]{pics/baad/F2015.png}
%   }
% \end{frame}

%-----------------------------------

\begin{frame}\frametitle{{\color{b-pink} 31 variables} for individual plants}
\begin{tikzpicture}[remember picture, overlay]
  \node[anchor = south, align=center, fill=white, rounded corners,
  draw=b-green, line width=2pt] at ($(current page.south) +(1, 0.25cm)$) {
    \includegraphics<1>[width=0.7\textwidth]{figures/baad_variable_count.pdf}
   };
  \end{tikzpicture}
\end{frame}

%-----------------------------------
\begin{frame}\frametitle{ Why (1): \color{b-blue} I needed data}
\begin{tikzpicture}[remember picture, overlay]
  \node[anchor = north, align=center, fill=white, rounded corners,
  draw=b-orange, line width=2pt] at ($(current page.south) +(0,8cm)$) {
    \includegraphics<1>[width=0.6\textwidth]{figures/tradeoff-Falster2015-1.pdf}
    \includegraphics<2>[width=0.6\textwidth]{figures/tradeoff-Falster2015-2.pdf}
    \includegraphics<3>[width=0.6\textwidth]{figures/tradeoff-Falster2015-3.pdf}
   };
  \end{tikzpicture}
\end{frame}

%-----------------------------------
{\setbeamercolor{background canvas}{bg=black}
\setbeamercolor{frametitle}{bg=black}
\begin{frame}\frametitle{Why (2): \color{b-orange} New workflow}
\begin{tikzpicture}[remember picture, overlay]
  \node[anchor = north, align=center] at ($(current page.south) +(0,8.5cm)$) {
    \includegraphics<1-2>[width=0.55\textwidth]{downloads/BTFF15.jpg}
   };
  \node[anchor = west, align=center, fill=white] at ($(current page.west) +(0,2cm)$) {
    \includegraphics[width=0.15\textwidth]{pics/cut-copy-paste_icon}
   };
  \node[anchor = west, align=center, fill=white] at ($(current page.west) +(0,-0.5cm)$) {
    \includegraphics[width=0.15\textwidth]{pics/Excel_icon}
   };
  \node[anchor = west, align=center, fill=white] at ($(current page.west) +(0,-3cm)$) {
    \includegraphics[width=0.15\textwidth]{pics/Email_icon}
   };

\only<2>{
  \node[anchor = east, align=center, fill=white] at ($(current page.east) +(0,2cm)$) {
    \includegraphics[width=0.15\textwidth]{pics/Octocat.png}
   };
  \node[anchor = east, align=center, fill=white] at ($(current page.east) +(0,-0.5cm)$) {
    \includegraphics[width=0.15\textwidth]{pics/rstudio.png}
   };
  \node[anchor = east, align=center, fill=white] at ($(current page.east) +(0,-3cm)$) {
    \includegraphics[width=0.15\textwidth]{pics/travis-mascot.png}
   };
}
\end{tikzpicture}
\end{frame}
}

%-----------------------------------

\begin{frame}
  \herohigh{Lots of {\color{b-pink} small data}}
  \bottomhanging{
    \includegraphics[width=0.9\paperwidth]{pics/baad/Collage}
  }
\end{frame}

\begin{frame}
  \herohigh{{\color{b-blue} \#otherpeoplesdata}}
  \begin{tikzpicture}[remember picture, overlay]
  \node[anchor = south, align=center] at ($(current page.south) +(0, 2cm)$) {
      \includegraphics[width=.85\paperwidth]{shots/otherpeoplesdata.png}
 };
  \end{tikzpicture}
\end{frame}

%-----------------------------------

\begin{frame}\frametitle{{\color{b-blue}General challenges} } % of {\color{b-pink} building BAAD}
%       1. & Harmonise fragments\\
%       2. & Versioned releases\\
%       3. & Reproducible research \\
%       4. & Entirely free \\
%       6. & Invites crowd sourcing\\

\begin{tikzpicture}[remember picture, overlay]%
  \node [anchor=north, align=left] at
  ($(current page.center) + (0, 0.3\paperheight)$) {%
    \includegraphics<1>[width=1\paperwidth]{diagrams/challenge_baad_1}
    \includegraphics<2>[width=1\paperwidth]{diagrams/challenge_baad_2}
    \includegraphics<3>[width=1\paperwidth]{diagrams/challenge_baad_3}
 };
\end{tikzpicture}
\end{frame}

%-----------------------------------

\begin{frame}
  \herohigh{ {\color{b-green}Harmonising}}
  \begin{tikzpicture}[remember picture, overlay]%
    \node [anchor=north, align=left] at
    ($(current page.center) + (0, .17\paperheight)$) {%
    % \only<1>{\includegraphics[width=0.9\paperwidth]{pics/baad/Collage}}
    \only<1>{
      \begin{tabular}{c l}
        1. & Start open\\
        2. & Script everything\\
        3. & Pipeline for processing\\
        4. & Version control \\
        5. & Collaborate\\
        6. & Automate\\
        7. & Review process\\
      \end{tabular}
    }
  };
  \end{tikzpicture}
\end{frame}


\begin{frame}
  \herohigh{\color{b-purple} 1. Start open}
    \begin{tikzpicture}[remember picture, overlay]%
    \node[anchor=north west, align=left, text width=12cm] at
    ($(current page.west) + (0.05\paperwidth, 0.05\paperheight)$) {
      \footnotesize
        {\color{b-blue} Daniel:} {\color{white}Would you like to co-author a data paper in Ecology? }\\

        {\color{b-pink} Yes {\FA \faArrowRight}}  {\color{white}Great, tell us what you've got.} \\

        {\color{b-pink} No {\FA \faArrowRight}}  {\color{white} No worries. Get back to us if you change your mind.}\\
    };

    % To do: add license pics: CC0, CC-BY
  \end{tikzpicture}
\end{frame}

\begin{frame}
  \herohigh{\color{b-purple} 2. Script everything}

  \begin{tikzpicture}[remember picture, overlay]%
    \node[anchor=north, align=center] at
    ($(current page.center) + (0, 0.17\paperheight)$) {
    \normalsize {\FA \faThumbsUp} Transparent, extensible, distributed};
    \node[anchor=north, align=center] at
    ($(current page.center) + (0, 0\paperheight)$) {
    \includegraphics<1>[width=.65\paperwidth]{diagrams/challenge_baad_0}
    \includegraphics<2>[width=.95\paperwidth]{snippets/baad_rebuild}
    };
  \end{tikzpicture}
\end{frame}


\begin{frame}
  \herohigh{\color{b-pink} 3. Pipeline}

  \begin{tikzpicture}[remember picture, overlay]%
    \node [anchor=north, align=left] at
    ($(current page.center) + (0, .17\paperheight)$) {%
      \begin{tabular}{c l}
        & Change units\\
        & Change variable names \\
        & Record methods\\
        & Don't modify raw data files \\
        & Encode meta-data as data \\
      \end{tabular}
  };
  \end{tikzpicture}
\end{frame}

\begin{frame}
  \minionhigh{  {\color{b-purple} 4. Version control} {\color{white} \&}
      {\color{b-orange}  5. Collaboration}}%
  \lowerhalf{
    \LARGE {\color{white} R + {\color{b-purple} git} +
          {\color{b-orange}GitHub} =} {\color{b-pink}\heart}%
  }
\end{frame}

\begin{frame}
  \only<1>{\minionhigh{Work on same code base\phantom{g}}}%
  \only<2>{\minionhigh{See what changed}}%
  \bottomhanging{%
    \includegraphics<1>[width=0.9\paperwidth]{shots/version_control_collaboration}%
    \includegraphics<2>[width=0.9\paperwidth]{shots/version_control_changes}
    }
\end{frame}


\begin{frame}
  \minionhigh{{\LARGE6. Automate}
  \\[0ex]\color{b-green} \hspace{2em} (Continuous Integration)}
  \lowerhalf{
    \begin{minipage}{\textwidth}
      \begin{enumerate}
      \item Commit changes
      \item Push to GitHub
      \item Get an email if something breaks
      \end{enumerate}
    \end{minipage}
  }
  \note{}
\end{frame}

% \begin{frame}
%   \minionhigh{%
%     \only<1>{Runs on virtual machine\ldots}}
%   \bottomhanging{%
%     \includegraphics<1>[width=\paperwidth]{shots/travis_spinup}%
%   }
% \end{frame}

% \begin{frame}
%   \minionhigh{Set and forget -- never gets bored}
%   \bottomhanging{
%     \includegraphics[width=0.65\paperwidth]{shots/travis_builds}%
%   }
%   \bottomleft{\href{https://travis-ci.org/dfalster/baad/builds}{travis-ci.org/dfalster/baad/builds}}
% \end{frame}

\begin{frame}
  \herohigh{{\color{b-green} 7. Reviews } \color{white}  (via knitr)}
  \bottomhangingborder{
    \includegraphics[width=.75\paperwidth]{downloads/data_review_error.png}
  }
\end{frame}

%-----------------------------------

\begin{frame}
  \herohigh{\color{b-orange} Distribute}
  \bottomhanging{
    \includegraphics<1>[width=.85\paperwidth]{pics/baad/F2015.png}
    \includegraphics<2>[width=1\paperwidth]{diagrams/challenge_baad_3}
  }
\end{frame}

%-----------------------------------

\begin{frame}
  \herohigh{\color{b-orange} Distribute \color{white} as package}
  \bottomhanging{
    \includegraphics[width=.95\paperwidth]{snippets/baad_package}
  }
\end{frame}

%-----------------------------------

\begin{frame}
  \herohigh{ \resizebox{.9\paperwidth}{!}{Use package {\color{b-orange} datastorr}}}
  \vspace{3em}
    \begin{itemize}
      \color{b-green}
      \item Fetches {\color{b-orange}``releases''} from {\color{b-pink} github}
      \item {\color{b-pink}20 lines} to setup for BAAD
      \item Another example: \color{b-pink} \href{https://github.com/traitecoevo/taxonlookup}{taxonlookup}\\
    \end{itemize}
\end{frame}

%-----------------------------------

\begin{frame}
  \herohigh{\color{b-pink} Reproduce}
  \bottomhanging{
    \includegraphics[width=1\paperwidth]{diagrams/challenge_baad_3}
 }
\end{frame}

%-----------------------------------

% \begin{frame}
%   \herohigh{Package \color{b-pink} remake}
%  \begin{tikzpicture}[remember picture, overlay]%
%     \node [anchor=north, align=left] at
%     ($(current page.center) + (0, 0.1\paperheight)$) {%
%       \begin{tabular}{l l}
%         \color{b-blue} Targets & \color{white} Objects of interest\\
%         \color{b-orange}Rules & \color{white}Functions returning targets\\
%         \color{b-blue}Dependencies & \color{white}Upstream targets\\
%       \end{tabular}
%   };
%   \end{tikzpicture}
% \end{frame}

\begin{frame}
  \herohigh{\color{b-pink} Caches intermediates}

  \begin{tikzpicture}[remember picture, overlay]%
    \node[anchor=west, align=center] at
    ($(current page.west) + (0, -0.2\paperheight)$) {
    \includegraphics<1>[width=1.2\paperwidth]{snippets/remake_make_1}
    \includegraphics<2>[width=1.2\paperwidth]{snippets/remake_make_3}
    \includegraphics<3>[width=1.2\paperwidth]{snippets/remake_make_2}
    };
  \end{tikzpicture}
  \end{frame}

%-----------------------------------

\begin{frame}\frametitle{ {\color{b-pink} Thanks to collaborators}}

  \begin{tikzpicture}[remember picture, overlay]%
    \node [anchor=north, align=left] at
    ($(current page.center) + (0, .4\paperheight)$) {%
    {\small
      \begin{tabular}{c >{\color{white}}l  c >{\color{white}}l }
        \raisebox{-.03\paperheight}{%
          \includegraphics[height=.12\paperheight]{pics/people/FitzJohn}}&
        Rich FitzJohn  &
        \raisebox{-.03\paperheight}{%
          \includegraphics[height=.12\paperheight]{pics/people/Duursma}}&
        Remko Duursma  \\
        \raisebox{-.03\paperheight}{%
          \includegraphics[height=.12\paperheight]{pics/people/Barneche}}&
        Diego Barneche &
        \raisebox{-.03\paperheight}{%=
          \includegraphics[height=.12\paperheight]{pics/people/Ishihara}}&
        Masae Ishihara \\
     \end{tabular}
      }
  };
  \node [anchor=south, align=left, text width = 12cm] at
    ($(current page.south) + (0, 0.05cm)$) {%
   {\baselineskip=2.5pt\fontsize{9pt}{9pt} \selectfont  \color{white} \\
   {\color{b-blue}Data contributors:} Masahiro Aiba, Makoto Ando, Niels Anten, Michael Aspinwall, Jennifer Baltzer, Christopher Baraloto, Michael Battaglia, John Battles, Ben Bond-Lamberty, Michiel van Breugel, James Camac, Yves Claveau, Lluís Coll, Masako Dannoura, Sylvain Delagrange, Jean-Christophe Domec, Farrah Fatemi, Wang Feng, Veronica Gargaglione, Yoshiaki Goto, Akio Hagihara, Jefferson Hall,  Steve Hamilton, Degi Harja, Tsutom Hiura, Robert Holdaway, Lindsay Hutley, Tomoaki Ichie, Eric Jokela, Anu Kantola, Jeff Kelly, Tanaka Kenzo, David King, Brian Kloeppel, Takashi Kohyama, Akira Komiyama, Jean-Paul Laclau, Christopher Lusk, Douglas Maguire, Guerric le Maire, Annikki M{\"a}kel{\"a}, Lars Markesteijn, John Marshall, Katherine McCulloh, Itsuo Miyata, Karel Mokany, Shigeta Mori, Randall Myster, Masahiro Nagano, Shawna Naidu, Yann Nouvellon, Anthony O'Grady, Kevin O'Hara, Toshiyuki Ohtsuka, Noriyuki Osada, Olusegun Osunkoya, Pablo Luis Peri, Any Mary Petritan, Lourens Poorter, Angelika Portsmuth, Catherine Potvin, Johannes Ransijn, Douglas Reid, Sabina Ribeiro, Scott Roberts, Rolando Rodríguez, Angela Salda{\~n}a-Acosta, Ignacio Santa-Regina, Kaichiro Sasa, Galia Selaya, Stephen Sillett, Frank Sterck, Kentaro Takagi, Takeshi Tange, Hiroyuki Tanouchi, David Tissue, Toru Umehara, Hajime Utsugi, Matthew Vadeboncoeur, Fernando Valladares, Petteri Vanninen,  Angelica V{\aa}rhammar, Jian Wang, Elizabeth Wenk, Richard Williams, Fabiano de Aquino Ximenes, Atsushi Yamaba, Toshihiro Yamada, Takuo Yamakura, Ruth Yanai, Robert York \par
}
   };

  \end{tikzpicture}
\end{frame}

\begin{frame}
  \herohigh{\color{b-blue}\resizebox{.9\paperwidth}{!}{Work flow for data compilations}}
  \begin{tikzpicture}[remember picture, overlay]%
    \node[anchor=north west,align=left,font=\small] at
    ($(current page.west) + (0.05\paperwidth, .2\paperheight)$) {
      \begin{tabular}{>{\color{white}}l>{\color{white}}l >{\color{b-grey}\footnotesize}l}
    {\color{b-green} Harmonise} &
        More info & Paper &
        \hrefp{http}{doi.org/10.1890/14-1889.1}\\
            &  Blog post &
        \hrefp{http}{ropensci.org/blog/2015/06/03/baad/}\\
       BAAD &
        \hrefp{https}{github.com/dfalster/baad}\\
        &     remake &
        \hrefp{https}{github.com/richfitz/remake}\\
     {\color{b-orange} Distribute}&
      baad.data &
        \hrefp{https}{github.com/traitecoevo/baad.data}\\
      & datastorr &
        \hrefp{https}{github.com/richfitz/datastorr}\\
    {\color{b-pink} Reproduce} &
      remake &
        \hrefp{https}{github.com/richfitz/remake}\\
    &  Analysis &
        \hrefp{https}{github.com/RemkoDuursma/baadanalysis}\\
   \end{tabular}
  };
  \node[anchor=north east, fill= white, inner sep=1pt] at
    ($(current page.east) + (0, 0.5\paperheight)$) {
      \includegraphics[height=.15\paperheight]{pics/userR.png}
      };
  \end{tikzpicture}
\end{frame}

\end{document}

%%% Local Variables:
%%% mode: latex
%%% TeX-PDF-mode: t
%%% TeX-engine: xetex
%%% End:
